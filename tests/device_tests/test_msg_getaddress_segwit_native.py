...
VECTORS_BITCOIN = (  # coin, path, script_type, address
    (
        "Testnet",
        "84'/1'/0'/0/0",
...
VECTORS_ALTCOIN = (  # coin, path, script_type, address
    (
        "Groestlcoin",
        "84'/17'/0'/0/0",
        proto.InputScriptType.SPENDWITNESS,
...
@pytest.mark.parametrize("coin, path, script_type, address", VECTORS_BITCOIN)
...
@pytest.mark.altcoin
@pytest.mark.parametrize("show_display", (True, False))
@pytest.mark.parametrize("coin, path, script_type, address", VECTORS_ALTCOIN)
def test_show_segwit_altcoin(client, show_display, coin, path, script_type, address):
    assert (
        btc.get_address(
            client,
            coin,
